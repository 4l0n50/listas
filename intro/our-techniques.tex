...
$1') \vecb{b}\in \{0,1\}^\lb$ and $3') \forall i \in [m]: \vecb{d}_i=\begin{pmatrix} \vecb{u}_1 &   \vecb{u}_2 \end{pmatrix}   \smallpmatrix{b_i  \\ r_i}$,
...
Define the matrix $\matr{B}=(\vecb{b}_1|| \ldots || \vecb{b}_n) \in \{0,1\}^{\lb \times n}$ and let $\vecb{b}_i^*$ be the $i$th row of $\matr{B}$. To prove $([\vecb{c}_1]_1,\ldots, [\vecb{c}_n]_1) \in\Lang_{[\matr{M}]_1,[\matr{N}]_1,\matr{\Lambda},\grkb{\alpha}}^n$, we first compute MP commitments $[\vecb{d}_i]_1$, $i \in [n]$, to $\vecb{b}_i^*$.  As before, 
...
\item $\exists \vecb{w}_1,\ldots,\vecb{w}_n \in\Z_q^\lc, \widetilde{\matr{B}} \in \Z_q^{\lb \times n}$, (whose rows are denoted as $\widetilde{\vecb{b}}_i^*$, $i \in [m]$, and the columns $\widetilde{\vecb{b}}_j$, $j \in [n]$), such that  
   $2') \smallpmatrix
{
    \vecb{c}\\
    \grkb{\alpha}
...
\smallpmatrix
{
    \vecb{b}_j\\
    \vecb{w}_j
}$ and $3') \forall i \in [m]: \vecb{d}_i=\pmatri{ \vecb{u}_1 &   \vecb{u}_2}   \smallpmatrix{
\widetilde{b}^*_i  \\ r_i}$.
\end{itemize}
