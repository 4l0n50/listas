...
\end{equation}
 where $\boldsymbol \alpha  \in \Am_1^{m_y}$, $\boldsymbol \beta  \in \Am_2^{m_x}$, $\matr{\EscQE}=(\escQE_{i,j}) \in \Z_q^{m_x\times m_y}$, $t \in \Am_T$, and $\Am_1,\Am_2,\Am_T\in\{\GG_1,\GG_2,\GG_T\}$ 
%are $\Z_q$-vector spaces
...
We give more details about GS proofs in Appendix \ref{app:gs-proofs}. Next, we introduce the GS commitment scheme.
